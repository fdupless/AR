...
subjects = (
    [str(100 + i) for i in range(1, 8)]
    + ["109"]
    + ["101opt", "105opt", "106opt", "109opt"]
)
sub = ["108", "108opt"]
activities = np.array(
    [
...
def gettrainData(subjects, act, q, TimeToAnalyze=tinterval, outputRAW=False):
    ...
    for subID in subjects:
        df = pd.read_csv(
            "PAMAP2_Dataset/Protocol/subject" + subID + ".dat", delimiter=" "
        )  # dataframe of subject num
        df = df.fillna(
            method="ffill", axis=0
        )  # fills missing data with the next valid entry
...
def bogusdata(t):
    ...
    df2 = pd.read_csv(
        "PAMAP2_Dataset/Protocol/subject" + "108opt" + ".dat", delimiter=" "
    )
    ...
    testdata = np.append(
        np.append(
            testsit.data[start : start + 500],
            teststand.data[start : start + 500],
            axis=0,
        ),
        testwalk.data[start : start + walkt],
        axis=0,
    )
    p = 4
    testdataX = np.append(
        np.append(
            testsit.data[start : start + 500, p], teststand.data[start : start + 500, p]
        ),
        testwalk.data[start : start + walkt, p],
    )
    p = 5
    testdataY = np.append(
        np.append(
    ...
    timestand = (
        teststand.data[start : start + 500, 0]
        - teststand.data[start, 0]
        + timesit[-1]
        + 0.01
    )
    timewalk = (
        testwalk.data[start : start + walkt, 0]
        - testwalk.data[start, 0]
        + timestand[-1]
        + 0.01
    )
...
def run_simulation(
    name="Simulation", clfname="RFC_activities", TimeToAnalyze=tinterval
):
    ...
    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=totaltime * 100, interval=10, blit=True
    )
...
def confusionmatrix(
    dataX, dataY, name="confmat", clfname="RFC_activity", acts=activities
):
    ...
    cbar.set_label(
        r"(# Classification)/ (Total Row # Classification)", rotation=90, size=23
    )
    ...
    if "train" in sys.argv:
        ...
        if ncores > 1:
            ...
            for i in range(ncores - 1):
                ...
                p = Process(
                    target=gettrainData, args=(subjects[M * i : M * (i + 1)], acts, q)
                )
                p.start()
